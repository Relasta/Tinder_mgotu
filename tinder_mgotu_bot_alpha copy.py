...
def on_click_menu(message):
    if message.text == '📊Статистика':
        ...
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        btn3 = types.KeyboardButton('↩️ Назад')
        btn4 = types.InlineKeyboardButton('💌 Отправить сообщение!', url='https://t.me/+TMVJyFHqqJgxNjcy')
        markup.add(btn4, btn3)
        bot.send_message(message.chat.id, "Здесь ты можешь отправить отзывы и пожеланию админу бота. Просто напиши, что хочешь в чат одним сообщеним и отправь.", reply_markup=markup) 
        bot.register_next_step_handler(message, on_click_test)
