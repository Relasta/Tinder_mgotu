...
def start(message): # message -- вся инфа о пользователе в виде словаря
    ...
    btn4 = types.KeyboardButton('🥰 Перейти к сравнению') 
    btn3 = types.KeyboardButton('📋 Правила') 
    btn2 = types.KeyboardButton('💌 Пожелания') 
...
def on_click_menu(message):
    if message.text == '📊 Статистика':
        ...
        btn3 = types.KeyboardButton('↩️ Назад')
        markup.add(btn3)
        bot.send_message(message.chat.id, "Тебе показывают фото двух девушек, ты должен нажать на стрелку указывающую на девушку, которая тебе больше понравилась. Нажав на имя ты можешь перейти на профиль девушки на нашем портале. Удачи!", reply_markup=markup) 
        bot.register_next_step_handler(message, start)
        ...
        btn3 = types.KeyboardButton('↩️ Назад')
        markup.add(btn3)
        markup.add(types.InlineKeyboardButton('💌 Отправить сообщение!', callback_data='send'))
        bot.send_message(message.chat.id, "Здесь ты можешь отправить отзывы и пожеланию админу бота. Просто напиши, чтохечшь в чат одним сообщеним и отправь.", reply_markup=markup) 
        bot.register_next_step_handler(message, start)
